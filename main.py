...
def execute_prestartup_script():
    ...
    for custom_node_path in node_paths:
        ...
        for possible_module in possible_modules:
            ...
            if os.path.isfile(module_path) or module_path.endswith(
                    ".disabled") or module_path == "__pycache__":
                continue
...
if os.name == "nt":
    ...
    logging.getLogger("xformers").addFilter(
        lambda record: 'A matching Triton is not available' not in record.
        getMessage())
...
def prompt_worker(q, server):
    ...
    while True:
        ...
        print("Prompt executed in {:.2f} seconds".format(time.perf_counter() -
    ...
    def hook(value, total, preview_image):
        ...
        if preview_image is not None:
            server.send_sync(BinaryEventTypes.UNENCODED_PREVIEW_IMAGE,
                             preview_image, server.client_id)
...
if __name__ == "__main__":
    ...
    threading.Thread(target=prompt_worker, daemon=True, args=(
        q,
        server,
    )).start()
